/* Intro — d3 line-art globe → flat map hand-off to the dashboard */
...
export default function IntroPage() {
  ...
  const goDashboard = useCallback(() => {
    navigate('/dashboard?from=intro')
  }, [navigate])

  useEffect(() => {
    loadMalaysiaGeo().catch(() => {})
  }, [])
    ...
    function gratFeature(s) {
      if (s.alpha < 0.55) {
        ...
        return {
          ...
          coordinates: parallels.map((l) => l.coordinates).concat(keep.map((m) => m.coordinates)),
        }
      }
  ...
  return (
    <div ref={bgRef} style={{ position: 'fixed', inset: 0, background: '#0a0e0c', zIndex: 100, overflow: 'hidden' }}>
      ...
      <div className="meteors" aria-hidden="true">
        <span className="meteor m1" /><span className="meteor m2" />
        <span className="meteor m3" /><span className="meteor m4" />
      ...
      >
        <div style={{
          ...
          Malaysian residential property intelligence
        </div>
