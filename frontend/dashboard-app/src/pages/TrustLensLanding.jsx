...
import HeroGlobe from '@/components/trustlens/HeroGlobe'
import ScrollGlobe from '@/components/trustlens/ScrollGlobe'
import {
  ...
  Sparkline,
  DEMO,
} from '@/components/trustlens/TrustLensUI'
import baitListing from '@/assets/bait-listing.jpeg'
...
const Slide = ({ id, dark, raised, children, style }) => (
  <section id={id} data-screen-label={id} style={{
    position: 'relative', scrollMarginTop: 72,
...
/* Bold editorial deck-index. A real 10-slide sequence, so the number earns a
   large serif anchor instead of a timid mono label. */
const Kicker = ({ n, track, dark }) => (
  <div style={{ display: 'flex', alignItems: 'flex-end', gap: 'clamp(14px,2vw,22px)', marginBottom: 'clamp(22px,3vh,34px)' }}>
    <span style={{
      fontFamily: "'Cormorant Garamond',serif", fontWeight: 300, lineHeight: 0.74,
      fontSize: 'clamp(54px, 7.5vw, 104px)', letterSpacing: '-0.04em',
      color: dark ? C.earthLight : C.earth,
    }}>{n}</span>
    <div style={{ paddingBottom: 'clamp(6px,1vh,12px)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
        <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 13, fontWeight: 600, color: dark ? 'rgba(220,215,201,.45)' : C.muted }}>/ 10</span>
        <span style={{ width: 26, height: 2, background: dark ? C.earthLight : C.earth, display: 'inline-block', borderRadius: 2 }}/>
      </div>
      <div style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 13, fontWeight: 700, letterSpacing: '.18em', textTransform: 'uppercase', color: dark ? 'rgba(220,215,201,.78)' : C.mid, marginTop: 8 }}>{track}</div>
    </div>
  </div>
);

/* Worded marker for the off-sequence sections (no deck number). */
const Marker = ({ lead, track, dark }) => (
  <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 'clamp(20px,3vh,30px)', flexWrap: 'wrap' }}>
    <span style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 'clamp(15px,1.4vw,18px)', fontWeight: 800, letterSpacing: '.04em', textTransform: 'uppercase', color: dark ? C.earthLight : C.earth }}>{lead}</span>
    <span style={{ width: 40, height: 2, background: dark ? 'rgba(220,215,201,.3)' : C.border, borderRadius: 2 }}/>
    <span style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 12, fontWeight: 600, letterSpacing: '.18em', textTransform: 'uppercase', color: dark ? 'rgba(220,215,201,.6)' : C.mid }}>{track}</span>
  </div>
);

const H = ({ children, dark, size = 'clamp(36px, 5.4vw, 68px)', style }) => (
  <h2 style={{
    fontFamily: "'Cormorant Garamond',serif", fontWeight: 500, fontSize: size,
    lineHeight: 1.0, letterSpacing: '-0.02em', margin: 0, maxWidth: 17 + 'ch',
...
const Lead = ({ children, dark, style }) => (
  <p style={{
    fontFamily: "'DM Sans',sans-serif", fontSize: 'clamp(16px, 1.55vw, 19px)', lineHeight: 1.62,
    color: dark ? 'rgba(220,215,201,.74)' : C.mid, maxWidth: 640, margin: '22px 0 0', textWrap: 'pretty', ...style,
  }}>{children}</p>
);

const Panel = ({ children, dark, style, accent }) => (
  <div className={dark ? 'tl-panel tl-panel-dark' : 'tl-panel'} style={{
    background: dark ? 'rgba(255,255,255,.04)' : C.raised,
    border: `1px solid ${dark ? 'rgba(220,215,201,.14)' : C.border}`,
    borderTop: accent ? `3px solid ${accent}` : undefined,
    borderRadius: 14, padding: 26, ...style,
  }}>{children}</div>
);
...
const IconBox = ({ Icon, dark, color = C.earth }) => (
  <div style={{
    ...
    background: dark ? 'rgba(162,123,92,.18)' : 'rgba(162,123,92,.14)',
  ...
  return (
    <nav style={{
      ...
      <div className="tl-navlinks" style={{ display: 'flex', alignItems: 'center', gap: 26 }}>
        ...
        }}>View Demo <I.arrow size={15}/></Link>
      </div>
    </nav>
  );
};
    ...
    <div className="tl-hero-grid" style={{ position: 'relative', zIndex: 2, width: '100%', display: 'grid', gridTemplateColumns: 'minmax(0,1.05fr) minmax(0,.95fr)', gap: 'clamp(32px, 5vw, 72px)', alignItems: 'center' }}>
      <div>
        ...
        <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', alignItems: 'center' }}>
          <Link to={APP_HREF} style={ctaPrimary}>View Demo <I.arrow size={17}/></Link>
...
const Problem = () => (
  <Slide id="problem">
    ...
    <div className="tl-grid-3" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20, marginTop: 'clamp(40px,6vh,64px)' }}>
      {PROBLEMS.map((p, i) => (
        <Panel key={i} accent={C.earth} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
            <IconBox Icon={p.Icon}/>
            <span style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 40, fontWeight: 300, lineHeight: 1, color: C.border }}>{String(i + 1).padStart(2, '0')}</span>
          </div>
          <div>
            <div style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 20, fontWeight: 700, color: C.deep, marginBottom: 9, letterSpacing: '-0.01em' }}>{p.title}</div>
            <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 14.5, lineHeight: 1.6, color: C.mid, margin: 0 }}>{p.body}</p>
          </div>
        </Panel>
      ))}
    </div>
    {/* the cost of getting it wrong — oversized editorial stat band */}
    <div className="tl-stat-row" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', marginTop: 'clamp(48px,7vh,76px)', borderTop: `2px solid ${C.deep}` }}>
      {[['RM800–2,500', 'cost of one professional valuation'], ['3–5 days', 'typical turnaround time'], ['0', 'sentiment signals in buyer tools']].map(([a, b], i) => (
        <div key={i} style={{ padding: 'clamp(24px,3vh,34px) clamp(18px,2vw,30px) 0', borderRight: i < 2 ? `1px solid ${C.border}` : 'none' }}>
          <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 'clamp(32px,4.4vw,56px)', fontWeight: 600, color: C.deep, letterSpacing: '-0.03em', lineHeight: 0.98 }}>{a}</div>
          <div style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 14, fontWeight: 500, color: C.mid, marginTop: 12, maxWidth: '24ch' }}>{b}</div>
        </div>
      ))}
    </div>
  </Slide>
...
const BaitSwitch = () => (
  <Slide id="bait" dark>
    <Marker lead="The #1 trick" track="Problem in focus" dark/>
    ...
    <div className="tl-flow" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 18, marginTop: 48, alignItems: 'stretch' }}>
      ...
      <div style={bsCard}>
        ...
        <div style={{ borderRadius: 10, overflow: 'hidden', border: `1px solid ${C.border}`, marginTop: 16 }}>
          <img src={baitListing} alt="Property listing" style={{ width: '100%', height: 110, objectFit: 'cover', display: 'block' }}/>
...
const Solution = () => (
  <Slide id="solution" raised>
    ...
    <div className="tl-grid-3" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 'clamp(16px,2.2vw,28px)', marginTop: 'clamp(40px,6vh,60px)' }}>
      {CAPS.map(([t, b], i) => (
        <div key={i} style={{ display: 'flex', gap: 16, padding: '22px 0', borderTop: `2px solid ${C.deep}` }}>
          <span style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 38, fontWeight: 300, color: C.earth, lineHeight: 0.9, paddingTop: 2 }}>{String(i + 1).padStart(2, '0')}</span>
          <div>
            <div style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 17, fontWeight: 700, color: C.deep, letterSpacing: '-0.01em' }}>{t}</div>
            <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 14, lineHeight: 1.55, color: C.mid, margin: '7px 0 0' }}>{b}</p>
          </div>
        </div>
      ))}
    </div>
...
const Audience = () => (
  <Slide id="audience" raised>
    <Marker lead="Audience" track="Who it's for"/>
  ...
  <section id="closing" data-screen-label="10 Closing" style={{
    ...
    <div style={{ maxWidth: 860, margin: '0 auto', textAlign: 'center' }}>
      ...
      <div style={{ display: 'flex', gap: 14, justifyContent: 'center', flexWrap: 'wrap' }}>
        <Link to={APP_HREF} style={ctaPrimary}>Pilot the Trust Score API <I.arrow size={17}/></Link>
...
/* ===================================================================== */
export default function TrustLensLanding() {
  return (
